...
async def test_websocket_connection(url: str, use_ssl: bool = True):
    ...
    try:
        ...
        async with websockets.connect(
            ...
            print(f"   [OK] Sent: {json.dumps(connected_message)}")
            ...
            print(f"   [OK] Sent: {json.dumps(start_message)}")
            ...
            try:
                ...
                print("   [WARN] No response received within 5 seconds (this is OK if server is just listening)")
            ...
            print(f"   [OK] Sent media event with {len(dummy_audio)} bytes of audio data")
            
            # Wait a bit more
            print("\n6. Waiting for any additional responses (3 seconds)...")
            try:
                response = await asyncio.wait_for(websocket.recv(), timeout=3.0)
                print(f"   [OK] Received: {response[:200]}...")
            except asyncio.TimeoutError:
                print("   [WARN] No additional response (this is OK)")
        ...
        print(f"\n[FAIL] Connection failed with HTTP status: {status_code}")
        if headers:
        ...
        print(f"\n[FAIL] Invalid WebSocket URL: {e}")
        print("  -> Check that the URL starts with 'wss://' or 'ws://'")
        return False
    except websockets.exceptions.ConnectionClosed as e:
        print(f"\n[FAIL] Connection closed by server: {e.code} - {e.reason}")
        return False
    except ssl.SSLError as e:
        print(f"\n[FAIL] SSL/TLS error: {e}")
...
async def main():
    ...
    if success:
        print("\n[SUCCESS] Test PASSED - WebSocket endpoint is accessible")
        print("\nNext steps:")
        ...
        print("\n[FAIL] Test FAILED - WebSocket endpoint is not accessible")
        print("\nTroubleshooting:")
        ...
        print("  5. If testing from server, try localhost: python test_websocket_connection.py wss://localhost/twilio --no-ssl-verify")
        sys.exit(1)
