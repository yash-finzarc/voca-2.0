...
async def handle_twilio_websocket(websocket: WebSocket):
    """
    Handle Twilio Media Streams WebSocket connection using Ultravox speech-to-speech.
    This endpoint uses Ultravox real-time speech-to-speech model (STS) instead of STT->LLM->TTS pipeline.
    """
    # #region agent log
    try:
    ...
    ultravox_client_dict = create_ultravox_client(
        api_key=ultravox_api_key,
    ...
    try:
        ...
        async def ultravox_audio_output_callback(pcm_audio: bytes):
            ...
            if not hasattr(ultravox_audio_output_callback, '_logged_diag'):
                ...
                ultravox_audio_output_callback._logged_diag = True
            ...
            async for message in websocket.iter_text():
                try:
                    ...
                    if event == "start":
                        ...
                        # Ultravox handles welcome message via system prompt - no need to send separately
                        # The system prompt is already configured when connecting to Ultravox
                        call_state = CallState.LISTENING
                        logger.info("[ULTRAVOX] Ready for real-time speech-to-speech conversation")
                        
                        # Commented out: Sarvam welcome message handling - Ultravox handles this via system prompt
                        ...
                        if not hasattr(ultravox_transcript_callback, '_first_media_logged'):
                            logger.info("[TWILIO] first media frame received")
                            ultravox_transcript_callback._first_media_logged = True
                        
                        media = data.get("media", {})
                        if media.get("track") == "inbound":
                            # Log media received after Ultravox audio (implicit playback confirmation)
                            if tts_audio_sent and not hasattr(ultravox_transcript_callback, '_media_after_audio_logged'):
                                logger.info("[TWILIO] Media received after Ultravox audio playback (playback confirmed)")
                                ultravox_transcript_callback._media_after_audio_logged = True
                            
                            # CRITICAL: Always forward audio to Ultravox if connected (full-duplex operation)
                            # Ultravox handles speech-to-speech in real-time
                            if not ultravox_client_dict.get("is_connected"):
                                logger.debug(f"[ULTRAVOX] Dropping audio: Ultravox not connected (state={call_state.value})")
                                continue
                            ...
                            while len(audio_buffer) >= BUFFER_SIZE:
                                ...
                                # Convert μ-law to PCM and forward to Ultravox (always, if Ultravox is connected)
                                try:
                                    pcm_audio = mulaw_to_pcm(mulaw_to_convert)
                                    await send_audio(ultravox_client_dict, pcm_audio)
                                    # Log first few audio frames to prove Ultravox continuity
                                    if not hasattr(ultravox_transcript_callback, '_audio_forwarded_logged'):
                                        logger.info(f"[ULTRAVOX] Audio frame forwarded to Ultravox (bytes={len(pcm_audio)}, state={call_state.value})")
                                        ultravox_transcript_callback._audio_forwarded_logged = True
                                except Exception as e:
                                    logger.error(f"[ULTRAVOX] Error forwarding audio to Ultravox: {e}", exc_info=True)
                            
                            # Commented out: Sarvam STT audio forwarding - using Ultravox instead
                        ...
                        # Send remaining audio to Ultravox
                        while len(audio_buffer) > 0:
                            ...
                            if mulaw_to_convert:
                                pcm_audio = mulaw_to_pcm(mulaw_to_convert)
                                if ultravox_client_dict.get("is_connected"):
                                    await send_audio(ultravox_client_dict, pcm_audio)
                        loop_exit_reason = "STOP event"
                        break
                        
                        # Commented out: Sarvam STT stop handling - using Ultravox instead
        ...
        try:
            ...
            logger.warning(f"[ULTRAVOX] Error stopping Ultravox: {e}")
        
        # Commented out: Sarvam STT/TTS cleanup - using Ultravox instead
