...
class VocaOrchestrator:
    ...
    async def process_user_text(self, text: str, session_id: str, organization_id: Optional[str] = None) -> str:
        """
        Async wrapper for processing user text input.
        This is the main entry point for the custom LLM pipeline.
        
        Args:
            text: User's text input (typically from STT transcription)
            session_id: Unique session identifier (typically CallSid)
            organization_id: Optional organization ID for system prompts
        
        Returns:
            Assistant's text response (to be sent to TTS)
        """
        # Use generate_reply which handles the full LLM pipeline
        # Wrap synchronous call in executor for async compatibility
        import asyncio
        loop = asyncio.get_event_loop()
        reply = await loop.run_in_executor(
            None,
            lambda: self.generate_reply(
                text,
