...
class SystemPromptRequest(BaseModel):
    ...
    id: Optional[str] = Field(
        None,
        description="UUID for the prompt (generated by frontend). Used for create/update/activate operations.",
    )
...
async def update_system_prompt(
    ...
    """Create or update the system prompt using UUID.
    ...
    If `id` is not provided, falls back to legacy behavior with organization_id.
    
    This aligns with frontend behavior where:
    - Frontend generates UUID using crypto.randomUUID()
    - Frontend calls create() or update() with UUID
    """
    try:
        # If UUID is provided, use UUID-based operations (new frontend approach)
        if request.id:
                ...
                if existing.data and len(existing.data) > 0:
                    ...
                    try:
                        ...
                        if success:
                            ...
                            return StatusResponse(status="success", message=f"System prompt updated successfully")
                        else:
                    ...
                    logger.info(f"Prompt {request.id} does not exist, creating new prompt...")
                    try:
                        ...
                        if success:
                            ...
                            return StatusResponse(status="success", message=f"System prompt created successfully")
                        else:
